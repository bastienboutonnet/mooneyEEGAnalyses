...
pathname_read='/Users/pss059/Documents/MATLAB/TutorialData/EEGLABHack1/';
pathname_write='/Users/pss059/Documents/MATLAB/TutorialData/EEGLABHack1/NightCrew/';
pathname_eventTracker='';
pathname_chanLocs='';
...
%ch3 = trig2: target
    ...
    EEG = pop_select( EEG,'nochannel',badChanDict('1')); %THIS MAY HAVE TO BE EXTENDED TO 4
    %load channel locs
    EEG = pop_chanedit(EEG, 'load',{[pathname_chanLocs,'nex_net.LOC'] 'filetype' 'loc'});
   
    if isequal(isKey(badChanDict,subject{s}),1)
        %exclude bad channels
        EEG = pop_select(EEG,'nochannel',badChanDict(subject{s}));
    ...
    %save set for postICA review
    EEG.setname = [subject{s} '_TRIFICA'];
    EEG = pop_saveset( EEG,  'filename', [pathname_write subject{s} '_TRIFICA.set']);
end
